...
class OutputPreparator:
            ...
            for i in range(len(members_with_parent)):
                for j in range(i + 1, len(members_with_parent)):
                    ...
                    text2 = OutputPreparator._get_node_by_id(id2, nodes)[
                        'text'].lower()
                    if text1 == text2 or OutputPreparator.gensim_repository.get_similarity_between_word(
    ...
    def topic_clustering_for_unstructured_result(json_output):
        ...
        best_k, best_cluster, center = OutputPreparator.clustering_repository.get_k_medoids_prediction(X_input)
        if best_k == -1:
          return json_output
    ...
    def _generate_X_for_k_medoids_clustering(array_of_nodes):
        ...
        for i, node in enumerate(array_of_nodes):
            word_embedded = OutputPreparator.gensim_repository.get_word_embedding_of_word(node['text'])
